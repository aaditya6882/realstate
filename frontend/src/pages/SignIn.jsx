  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center bg-slate-50 px-4 py-12 text-slate-800 transition-colors duration-300 dark:bg-slate-950 dark:text-slate-100">
      <form
        ...
        className="bg-white p-8 sm:p-10 rounded-3xl shadow-xl border border-slate-100 w-full max-w-md space-y-6 transition-colors duration-300 dark:bg-slate-900 dark:border-slate-800"
      >
        <div className="text-center">
          <div className="w-14 h-14 bg-amber-100 text-amber-700 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <LogIn className="w-7 h-7" />
          </div>
          <h2 className="text-3xl font-extrabold text-slate-900 mb-1 dark:text-slate-100">{t.welcomeBack}</h2>
          <p className="text-sm text-slate-500 font-medium dark:text-slate-400">{t.signInSub}</p>
        </div>

        {error && <p className="text-rose-500 text-xs font-bold text-center bg-rose-50 p-3 rounded-xl border border-rose-100 dark:bg-rose-950 dark:border-rose-900">{error}</p>}
